...
<dependency>
    ...
    <!-- 新版springboot和mybatisplus兼容性不够好，如果出现：
	Invalid value type for attribute 'facrotyBeanObjectType':java.lang.String
	需要排除依赖，引入新版mybatis-spring依赖-->
    <exclusions>
        <exclusion>
...
- 程序启动时：
  - 创建`DBUserDetailsManager`类，实现接口(可选，或者都实现) :
  
    ①UserDetailsManager, UserDetailsPasswordService (包含查询用户、新增、修改等方法)
  
    ②UserDetailsService(只包含根据用户名查询用户的方法)
...
Mybatis的user转为Spring的user，在调用spring的createUser方法，在把spring的user转为mybatis的user，然后存入数据库
...
为了减轻彩虹表的效果，开发人员开始使用加盐密码。不再只使用密码作为哈希函数的输入，而是为每个用户的密码生成随机字节（称为盐）。盐和用户的密码将一起经过哈希函数运算，生成一个唯一的哈希。盐将以明文形式与用户的密码一起存储。然后，当用户尝试进行身份验证时，盐和用户输入的密码一起经过哈希函数运算，再与存储的密码进行比较。唯一的盐意味着彩虹表不再有效，因为即使彩虹表的hash值和数据库的hash值一直，也无法获得密码，因为你要减掉盐值，才能获得密码，但是你不知道盐值是多少。

总结：

1.知道hash后，也就获得了：密码+盐，要获得密码，必须减去盐，但是不知道盐值是什么，因此无法求减，所以知道hash也没用

2.防止密码重复，不同用户有相同密码，但是盐值不同，hash结果也就不同



**自适应单向函数：**

即使hash+盐很安全，但是硬件发展也很迅速，攻击者可以通过 随机字符串1(猜测密码)+随机字符串2(猜测盐)，进行超速度运算，也有可能破解

现在，开发人员开始使用自适应单向函数来存储密码。使用自适应单向函数验证hash时，提高hash生成得复杂度`故意占用资源（故意占用大量的CPU、内存或其他资源）`让hash值没那么容易生成。自适应单向函数允许配置一个`“工作因子”`，随着硬件的改进而增加，从而提升复杂度。SpringSecurity建议将“工作因子”调整到系统中验证密码需要约一秒钟的时间。这种权衡是为了`让攻击者难以破解密码`。

自适应单向函数包括`bcrypt、PBKDF2、scrypt和argon2`。

总结：

1.hash暴力破解方式：攻击者可以通过 随机字符串1(猜测密码)+随机字符串2(猜测盐)，进行超速度运算，也有可能破解

2.自适应单项函数防止hash暴力破解方式：通过提高hash复杂度，让hash得生成很慢(如1秒)，攻击者只能通过相同得算法生成hash，此时即使硬件再好，每个hash需要很长时间生成，在超大量生成时，需要耗费超大量时间。

3.原理：通过 工作因子 决定hash生成时循环得次数，越高计算约复杂，攻击者必须使用相同得 工作因子 才能正确获得hash。工作因子 是公开得

### 6.2、PasswordEncoder

**BCryptPasswordEncoder**

特点：主要依赖于 CPU 的计算能力，越复杂对CPU得要求越高，对内存的消耗较低
...
特点：允许强行指定计算时分配的内存。并能控制并行度，既线程使用数量，能控制其能在多和CPU中性能，也可减少线程数量使破解速度提高
...
| 算法       | 增加复杂度的方式                             | 主要资源消耗                | 对并行计算的抗性                | 硬件加速抗性                 | 推荐用途                       |
| ---------- | -------------------------------------------- | --------------------------- | ------------------------------- | ---------------------------- | ------------------------------ |
| **bcrypt** | 增加迭代次数（工作因子）                     | 主要消耗 CPU 计算能力       | 较差，无法有效利用 GPU 并行计算 | 较低（适用于一般用途）       | 基本的密码哈希存储             |
| **scrypt** | 增加迭代次数、内存消耗和并行度（内存密集型） | 消耗大量内存和 CPU 计算资源 | 高，抗 GPU 和 ASIC 并行攻击     | 高（防止 GPU/ASIC 破解）     | 高安全性应用（如加密货币钱包） |
| **Argon2** | 增加迭代次数、内存消耗和并行度（非常灵活）   | 高内存消耗，支持多核 CPU    | 高，支持多线程并行计算          | 极高（专为防止硬件加速设计） | 高安全性应用，密码存储         |
| **PBKDF2** | 增加迭代次数（计算强度）                     | 主要消耗 CPU 计算资源       | 较差，抗 GPU 并行攻击能力较弱   | 较低（不支持内存密集型攻击） | 一般密码哈希存储（较旧的标准） |
...
.formLogin( form -> {
    form
        .loginPage("/login").permitAll() //登录页面无需授权即可访问，如果不添加这个，会无限重定向：login没权限，跳转登录页
...
public class MyAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
		//authentication:保存了用户 身份，角色，权限，密码等信息
        //获取用户身份信息
...
public class IndexController {
    ...
    public Map index(){
        ...
        Object credentials = authentication.getCredentials();//凭证(脱敏) TODO查看值时hash前还是hash后的
...
http.sessionManagement(session -> {
    session
        .maximumSessions(1) //设置最大允许几个账号同时登录 TODO 如何设置无限
        .expiredSessionStrategy(new MySessionInformationExpiredStrategy());//指定被挤掉(session失效)后，的处理方式 TODO默认方式是什么
});
```
